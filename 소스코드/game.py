...
#define font
font_small = pygame.font.SysFont('a소녀와뭉치', 20)
font_big = pygame.font.SysFont('a소녀와뭉치', 24)
...
score_image = pygame.image.load('assets/score.png').convert_alpha()
...
def draw_panel():
	draw_text('SCORE: ' + str(score), font_big, WHITE,10, 10)
...
class Player():
	def __init__(self, x, y):
		self.image = pygame.transform.scale(jumpy_image, (120, 120)) #캐릭터 크기
		self.width = 120
		self.height = 120
	...
	def move(self):
		...
		#process keypresses
		key = pygame.key.get_pressed() #버튼 설정
...
class Platform(pygame.sprite.Sprite):
	def __init__(self, x, y, width, moving):
		...
		self.image = pygame.transform.scale(platform_image, (width, 40)) #발판 두께
		self.moving = moving
...
while run:
	...
	if game_over == False:
		...
		if len(platform_group) < MAX_PLATFORMS:
			p_w = random.randint(100, 150) #발판 넓이
		...
		if fade_counter < SCREEN_WIDTH:
			...
			draw_text('SCORE: ' + str(score), font_big, WHITE, 165, 250)
