...
class Player():
	def __init__(self, x, y):
		self.image = pygame.transform.scale(jumpy_image, (50, 50)) #캐릭터 크기
		self.width = 50
		self.height = 50
...
while run:
	...
	for event in pygame.event.get():
		if event.type == pygame.QUIT:
			...
			run = False
		elif event.type == pygame.KEYDOWN:#필살기 구현 코드 
			if event.key==pygame.K_a:
#				if int(time.time())%2==0:#필살기 쿨타임 2초 
					enemy.kill()#새 없어짐(위치조정). 
